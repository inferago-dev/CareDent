...
export default function Navbar({ onOpenQuoteModal }) {
  ...
  /*
   * Navbar background strategy:
   *
   * The site has both dark-hero pages (/) and white-bg pages (/contact, /about…).
   * A fully transparent navbar puts white text on a white background on those pages.
   *
   * Solution: always apply a top-of-page gradient that fades from dark → transparent,
   * giving the white nav text a dark backdrop without covering the page content.
   * Once the user scrolls down, switch to a frosted-glass dark panel.
   */
  const headerBg = scrolled
    ? 'bg-blue-950/90 backdrop-blur-xl border-b border-white/10 shadow-lg shadow-blue-950/20'
    : 'bg-gradient-to-b from-blue-950/70 via-blue-950/30 to-transparent backdrop-blur-sm';
    ...
    <>
      ...
      <header className={`fixed w-full top-0 z-40 transition-all duration-500 ${headerBg}`}>
          ...
          <div className="flex items-center justify-between h-20">
              ...
              {navLinks.map((link) => {
                  ...
                  return (
                    ...
                    >
                      <button
                        ...
                        className={`flex items-center gap-1 text-sm font-medium tracking-tight transition-colors ${active ? 'text-cyan-400' : 'text-white/90 group-hover:text-cyan-400'
                          }`}
                      >
                        {link.name}
                        <ChevronDown
                ...
                return (
                  <Link
                    key={link.name}
                    to={link.path}
                    className={`text-sm font-medium tracking-tight transition-colors ${active ? 'text-cyan-400' : 'text-white/90 hover:text-cyan-400'
                      }`}
                  >
                    {link.name}
                  </Link>
            ...
            <div className="hidden lg:flex items-center gap-6">
              ...
              {isAuthenticated ? (
                <div className="relative" ref={accountMenuRef}>
                  <button
                    ...
                    aria-haspopup="menu"
                    className="flex items-center gap-2 text-sm font-medium text-white/90 hover:text-cyan-400 transition-colors"
                  >
                ...
                <Link
                  to="/login"
                  className="flex items-center gap-1.5 text-sm font-medium text-white/90 hover:text-cyan-400 transition-colors"
                >
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-full bg-cyan-600 hover:bg-cyan-500 text-white text-sm px-5 py-2.5 transition-all duration-300 hover:-translate-y-0.5 shadow-lg shadow-cyan-600/20 hover:shadow-cyan-500/30 active:scale-95"
              >
            ...
            <div className="flex lg:hidden items-center gap-1">
              <button
                ...
                className="p-2 text-white/90 hover:text-cyan-400 transition-transform duration-200"
        ...
        {shouldRenderMobileMenu && (
          <div
            id="mobile-nav"
            className={`lg:hidden border-t border-white/10 backdrop-blur-2xl bg-blue-950/85 px-6 pt-4 pb-8 space-y-6 origin-top max-h-[calc(100dvh-5rem)] overflow-y-auto overscroll-contain ${mobileMenuOpen ? 'animate-drop-in' : 'animate-drop-out'}`}
          >
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    key={link.name}
                    to={link.path}
                    className={`block py-3 text-base font-medium transition-colors ${active ? 'text-cyan-400' : 'text-white/80 hover:text-cyan-400'
                      }`}
                  >
                    {link.name}
                  </Link>
            ...
            <div className="pt-4 border-t border-white/10 space-y-3">
              {isAuthenticated ? (
                <>
                  ...
                  <Link
                    to={isAdmin ? '/admin' : '/portal'}
                    className="w-full flex items-center justify-center gap-2 rounded-full border border-white/20 py-2.5 text-white/80 font-medium text-sm hover:border-cyan-400 hover:text-cyan-400 transition-colors"
                  >
                    <LayoutDashboard className="w-4 h-4" />
                    <span>{isAdmin ? 'Admin Dashboard' : 'My Account'}</span>
                  </Link>
              ...
              <button
                ...
                className="w-full rounded-full bg-cyan-600 hover:bg-cyan-500 text-white font-medium py-3 flex items-center justify-center gap-2 transition-colors"
              >
