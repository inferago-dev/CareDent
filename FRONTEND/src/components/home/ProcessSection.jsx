    ...
    <section className="relative section-y bg-slate-50 border-y border-slate-200">
      <div className="container-page max-w-7xl">
        <div className="max-w-2xl">
          ...
          <Reveal delay={160}>
            <p className="text-slate-500 mt-4 leading-relaxed tracking-tight">
        ...
        <ol className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2 mt-14">
          {STEPS.map((step, idx) => {
            const Icon = step.icon;
            return (
              <Reveal key={step.title} delay={idx * 100} y={28} duration={850}>
                <li className="group h-full list-none bg-white border border-slate-200 rounded-2xl p-6 space-y-4 hover:border-cyan-200 hover:shadow-xl hover:shadow-cyan-900/5 transition-all duration-500">
                  <div className="flex items-center justify-between">
                    <div className="w-11 h-11 rounded-xl bg-cyan-100 text-cyan-600 flex items-center justify-center group-hover:bg-cyan-600/30 transition-colors duration-500">
                  ...
                  <h3 className="text-base font-medium text-slate-900 leading-tight tracking-tight">{step.title}</h3>
                  <p className="text-sm text-slate-500 leading-relaxed tracking-tight">{step.body}</p>
                </li>
              </Reveal>
            );
          })}
